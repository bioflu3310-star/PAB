...
export default function FormBuilder() {
  ...
  function addFieldAt(idx, type) {
    ...
    setFields(prev => { const next = [...prev]; next.splice(idx, 0, newField); return next })
    setOpenMenu(null)
  }
  ...
  function moveField(idx, dir) {
    ...
    setFields(prev => { const next = [...prev]; [next[idx], next[ni]] = [next[ni], next[idx]]; return next })
  }

  function duplicateField(idx) {
    setFields(prev => { const next = [...prev]; next.splice(idx + 1, 0, { ...JSON.parse(JSON.stringify(prev[idx])), id: Date.now() }); return next })
  }
  ...
  function AddButton({ idx }) {
    ...
    return (
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, paddingLeft: 13, height: 28, margin: '-6px 0', opacity: isOpen ? 1 : 0.5, position: 'relative', zIndex: isOpen ? 150 : 1, transition: 'opacity 0.15s' }}
        onMouseEnter={e => e.currentTarget.style.opacity = 1} onMouseLeave={e => { if (!isOpen) e.currentTarget.style.opacity = 0.5 }}>
        <button onClick={() => setOpenMenu(isOpen ? null : idx)}
          style={{ width: 26, height: 26, borderRadius: '50%', background: 'var(--accent)', color: '#fff', border: 'none', cursor: 'pointer', fontSize: 16, fontWeight: 700, display: 'flex', alignItems: 'center', justifyContent: 'center', boxShadow: '0 2px 6px rgba(79,142,247,.3)' }}>+</button>
        <span style={{ fontSize: 11, color: 'var(--text3)', fontStyle: 'italic' }}>Add field</span>
        {isOpen && (
          <div style={{ position: 'absolute', top: 32, left: 0, background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 8, boxShadow: '0 8px 32px rgba(0,0,0,.4)', padding: 6, zIndex: 9999, minWidth: 220, maxHeight: 320, overflowY: 'auto' }}>
            {Object.entries(FIELD_TYPES).map(([k, v]) => (
              <button key={k} onClick={() => addFieldAt(idx, k)}
                ...
                <span style={{ width: 22, height: 22, background: 'var(--accent-bg)', borderRadius: 5, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 12, color: 'var(--accent)' }}>{v.label.split(' ')[0]}</span>
                <div><div style={{ fontWeight: 600 }}>{v.label.substring(v.label.indexOf(' ') + 1)}</div><div style={{ fontSize: 10, color: 'var(--text3)' }}>{v.hint}</div></div>
              </button>
            ))}
          </div>
        )}
  ...
  return (
    <div className="fade-in" onClick={() => setOpenMenu(null)}>
      <div style={{ maxWidth: 860, margin: '0 auto' }}>
        <div className="page-header">
          <div className="page-title">{isEdit ? 'Edit Form' : 'Create New Form'}</div>
          <div className="page-sub">Type your question, pick the answer type, and see a live preview.</div>
        </div>
        ...
        <div className="fb-card">
          ...
          <div className="fb-body">
            {fields.length === 0 ? (
              <div style={{ textAlign: 'center', padding: 32, color: 'var(--text3)', background: 'var(--surface2)', borderRadius: 10, border: '1px dashed var(--border2)' }}>
                <div style={{ fontSize: 32, marginBottom: 8, opacity: 0.5 }}>📝</div>
                <div style={{ fontSize: 14, fontWeight: 600, marginBottom: 14 }}>No fields yet</div>
                <button className="btn btn-primary btn-sm" onClick={() => addFieldAt(0, 'text')}>⊕ Add First Field</button>
              </div>
            ) : (
              <>
                {fields.map((f, i) => (
                    ...
                    {f.type === 'section' ? (
                      <div style={{ background: 'linear-gradient(135deg,rgba(245,166,35,.08),rgba(245,166,35,.02))', border: '1px solid rgba(245,166,35,.25)', borderRadius: 10, padding: '14px 18px', marginBottom: 12, position: 'relative' }}>
                        <input value={f.label} onChange={e => updateField(i, 'label', e.target.value)}
                          placeholder="Section title…" style={{ background: 'transparent', border: 'none', fontFamily: "'Playfair Display',serif", fontSize: 16, fontWeight: 700, color: 'var(--text)', outline: 'none', width: '100%' }} />
                        <button onClick={() => removeField(i)} style={{ position: 'absolute', top: 10, right: 10, background: 'none', border: 'none', color: 'var(--danger)', cursor: 'pointer', fontSize: 14 }}>✕</button>
                      </div>
                    ) : (
                      <div className="fb-field-card">
                          ...
                          <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 8 }}>
                            ...
                            <div style={{ display: 'flex', gap: 8, alignItems: 'center', flexWrap: 'wrap' }}>
                              <select value={f.type} onChange={e => changeType(i, e.target.value)}
                                style={{ background: 'var(--input-bg)', border: '1px solid var(--border)', borderRadius: 6, padding: '6px 10px', fontSize: 12, color: 'var(--text)', minWidth: 140, fontFamily: "'Inter',sans-serif" }}>
                              ...
                              <label style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 12, cursor: 'pointer' }}>
                                <input type="checkbox" checked={f.required || false} onChange={e => updateField(i, 'required', e.target.checked)}
                                  style={{ accentColor: 'var(--accent)' }} /> Required
                              </label>
                              <div style={{ marginLeft: 'auto', display: 'flex', gap: 4 }}>
                                <button className="btn btn-ghost btn-xs" onClick={() => duplicateField(i)}>⎘</button>
                                <button className="btn btn-ghost btn-xs" disabled={i === 0} onClick={() => moveField(i, -1)}>▲</button>
                                <button className="btn btn-ghost btn-xs" disabled={i === fields.length - 1} onClick={() => moveField(i, 1)}>▼</button>
                                <button className="btn btn-danger btn-xs" onClick={() => removeField(i)}>🗑</button>
                              </div>
                            </div>
                          </div>
                        </div>
                        ...
                        <div style={{ paddingTop: 10, paddingLeft: 36, borderTop: '1px dashed var(--border)' }}>
                          <div style={{ fontSize: 10, fontWeight: 700, color: 'var(--text3)', letterSpacing: 1, textTransform: 'uppercase', marginBottom: 8 }}>Preview:</div>
                          {['text', 'textarea', 'number', 'email', 'date', 'time'].includes(f.type) && (
                            <input className="input" disabled placeholder={f.placeholder || 'Answer here…'} style={{ opacity: 0.6 }} />
                          )}
                          {['checkbox', 'radio', 'dropdown'].includes(f.type) && (
                            <div style={{ display: 'flex', flexDirection: f.type === 'radio' ? 'row' : 'column', flexWrap: 'wrap', gap: f.type === 'radio' ? 10 : 6 }}>
                              {(f.options || []).map((o, oi) => (
                                <div key={oi} style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                                  ...
                                  <input value={o} onChange={e => updateOption(i, oi, e.target.value)}
                                    ...
                                    style={{ flex: 1, background: 'transparent', border: 'none', borderBottom: '1px solid var(--border)', padding: '4px 2px', fontSize: 13, color: 'var(--text)', outline: 'none', minWidth: 80, fontFamily: "'Inter',sans-serif" }} />
                                  <button onClick={() => removeOption(i, oi)} style={{ background: 'none', border: 'none', color: 'var(--text3)', cursor: 'pointer', fontSize: 12 }}>✕</button>
                                </div>
                              ))}
                              <button onClick={() => addOption(i)} style={{ background: 'none', border: 'none', color: 'var(--accent)', fontSize: 12, cursor: 'pointer', textAlign: 'left', padding: '4px 0', fontFamily: "'Inter',sans-serif" }}>+ Add option</button>
                            </div>
                          )}
                        </div>
                      </div>
        ...
        <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 10, marginBottom: 28 }}>
          <button className="btn btn-ghost" onClick={() => navigate('/forms')}>Cancel</button>
          <button className="btn btn-ghost" onClick={() => save('draft')}>Save Draft</button>
          <button className="btn btn-primary" onClick={() => save('published')}>Save & Publish →</button>
        </div>
      </div>
    </div>
  )
